...
export function startStubOidc({ clientId, clientSecret, issuer, user, publicOrigin } = {}) {
  ...
  const server = http.createServer((req, res) => {
    requestLog.push({ url: req.url, method: req.method, host: req.headers.host });
    ...
    if (parsed.pathname === "/auth" && req.method === "GET") {
      ...
      if (query.get("state")) redirectUrl.searchParams.set("state", query.get("state"));
    ...
    if (parsed.pathname === "/token" && req.method === "POST") {
      ...
      req.on("end", () => {
        ...
        if (entry.codeChallenge && !pkceMatches(codeVerifier, entry.codeChallenge)) {
          res.writeHead(400, { "content-type": "application/json" });
