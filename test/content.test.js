...
async function publish(t, { zip = TWO_FILE_SITE, filename = "site.zip", title = "My Site" } = {}) {
  const cookie = t.signSession({ sub: "u1", name: "Test User", email: "t@example.invalid" });
  const { body, contentType } = buildMultipart([
    { type: "file", name: "file", filename, content: zip, contentType: "application/zip" },
...
test("GET / on the address host serves the fixture byte-for-byte with no session cookie", async () => {
  ...
  try {
    ...
    const res = await get(t.baseUrl, "/", { host: `${address}.handout.example.com` });
...
test("GET /assets/app.css on the address host serves the asset with the right type", async () => {
  ...
  try {
    const { address } = await publish(t);
    const res = await get(t.baseUrl, "/assets/app.css", { host: `${address}.handout.example.com` });
    assert.strictEqual(res.status, 200);
...
test("a wrapper-directory zip serves dist/index.html at / and dist/assets/app.css at /assets/app.css", async () => {
  const t = await buildTestServer();
  try {
    const { address } = await publish(t, { zip: WRAPPER_SITE, filename: "wrapper.zip" });
    const root = await get(t.baseUrl, "/", { host: `${address}.handout.example.com` });
    assert.strictEqual(root.status, 200);
    assert.ok(root.body.toString("utf8").includes("Wrapper site"));

    const asset = await get(t.baseUrl, "/assets/app.css", { host: `${address}.handout.example.com` });
...
test("GET /.handout is 404", async () => {
  ...
  try {
    const { address } = await publish(t);
    const res = await get(t.baseUrl, "/.handout", { host: `${address}.handout.example.com` });
...
test("a traversal attempt is 404 and reads nothing", async () => {
  ...
  try {
    const { address } = await publish(t);
    const res = await get(t.baseUrl, "/../../.env", { host: `${address}.handout.example.com` });
    assert.strictEqual(res.status, 404);
  } finally {
    await t.close();
  }
...
test("a PDF publish serves application/pdf with Content-Disposition: inline", async () => {
  ...
  try {
    ...
    const cookie = t.signSession({ sub: "u1", name: "Test User", email: "t@example.invalid" });
    const { body, contentType } = buildMultipart([
      { type: "file", name: "file", filename: "report.pdf", content: pdfBytes, contentType: "application/pdf" },
    ...
    const pdfRes = await get(t.baseUrl, "/", { host: `${address}.handout.example.com` });
    assert.strictEqual(pdfRes.status, 200);
    assert.strictEqual(pdfRes.headers["content-type"], "application/pdf");
    assert.match(pdfRes.headers["content-disposition"], /inline; filename="report\.pdf"/);
  } finally {
    await t.close();
  }
});
...
test("an unknown but address-shaped label is 404", async () => {
  const t = await buildTestServer();
  try {
    const res = await get(t.baseUrl, "/", { host: "abc2defgh3.handout.example.com" });
    assert.strictEqual(res.status, 404);
  } finally {
    await t.close();
  }
